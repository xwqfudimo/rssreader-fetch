...
//			System.out.println(wordsNum);
		...
		for (int i = 0; i < indexDistribution.size() - 1; i++) {
			if (indexDistribution.get(i) > threshold && ! boolstart) {
				if (indexDistribution.get(i+1).intValue() > threshold
					|| indexDistribution.get(i+2).intValue() > threshold) {
					boolstart = true;
					start = i;
					continue;
				}
			...
			if (boolstart) {
				if (indexDistribution.get(i).intValue() <threshold
					&& indexDistribution.get(i+1).intValue() < threshold 
					&& indexDistribution.get(i+2).intValue() < threshold) {
					end = i;
					boolend = true;
				}
...
//				System.out.println(start+1 + "\t\t" + end+1);
				for (int j = start; j <= end; j++) {
					if (lines.get(j).length() < 5) continue;
